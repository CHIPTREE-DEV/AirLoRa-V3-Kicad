...
PMS_status PMS_sleep(PMS_typedef *PMS_struct)
{
	...
	while(1)
	{
		...
		//__HAL_UART_CLEAR_FLAG(PMS_struct->PMS_huart, HAL_UART_ClearNoiseError(&huart1)|HAL_UART_ClearOverrunError(&huart1));
...
PMS_status PMS_read(PMS_typedef *PMS_struct)
{
	...
	if(!_sleepflag)
	{
		...
		case PMS_MODE_ACTIVE:
			while(1)
			{
				//__HAL_UART_CLEAR_FLAG(PMS_struct->PMS_huart, UART_CLEAR_NEF|UART_CLEAR_OREF);
				__HAL_UART_FLUSH_DRREGISTER(PMS_struct->PMS_huart);
